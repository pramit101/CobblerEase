      ...
      <SafeAreaView style={{ flex: 1 }}>
        <View style={styles.main}>
          <View style={styles.sub_heading}>
            <Text style={styles.sub_heading_text}>
              Track your order
            </Text>
          </View>
          ...
          />
          <TouchableOpacity style={localStyles.button} onPress={handleTrackOrder}>
          ...
          {foundOrder && (
            <ScrollView style={{ marginTop: 20 }}>
              <View style={localStyles.card}>
                <Text style={localStyles.item}>{foundOrder.items?.[0]?.productId || 'Service Order'}</Text>
                <Text style={localStyles.info}>Order ID: {foundOrder.id}</Text>
                <Text style={localStyles.info}>Placed: {foundOrder.date}</Text>
                <Text style={localStyles.info}>Estimated Delivery: {foundOrder.estimated}</Text>
                ...
                <View style={localStyles.progressContainer}>
                  {['Placed', 'Processing', 'Out for Delivery', 'Delivered'].map((step, index) => (
                    <View key={step} style={localStyles.step}>
                      <View style={[
                        localStyles.circle,
                        step === foundOrder.status || index < getStatusIndex(foundOrder.status) ? localStyles.activeCircle : null
                      ]} />
                      <Text style={[
                        localStyles.label,
                        step === foundOrder.status || index < getStatusIndex(foundOrder.status) ? localStyles.activeLabel : null
                      ]}>
                        {step}
                      </Text>
                    </View>
                ...
                <TouchableOpacity
                  ...
                  onPress={() => router.push({ pathname: "/order_details", params: { order: JSON.stringify(foundOrder) } })}
                >
...
const localStyles = StyleSheet.create({
  input: { borderWidth: 1, borderColor: '#ccc', borderRadius: 5, padding: 10, marginVertical: 15 },
  button: { backgroundColor: '#007bff', padding: 12, borderRadius: 5 },
  buttonText: { color: '#fff', textAlign: 'center', fontWeight: '600' },
  card: { padding: 15, backgroundColor: '#fff', marginBottom: 15, borderRadius: 10, elevation: 3 },
  item: { fontSize: 18, fontWeight: '600', marginBottom: 5, color: '#333' },
  info: { fontSize: 14, color: '#555', marginVertical: 1 },
  progressContainer: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 15 },
